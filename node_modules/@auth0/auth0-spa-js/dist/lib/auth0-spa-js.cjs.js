...
class AnonymousSessionCacheManager {
    constructor(clientId, cacheMode) {
        this.slots = new Map;
        this.baseKey = "".concat(STORAGE_KEY_PREFIX, "::").concat(clientId, "::anonymous");
        this.useLocalStorage = cacheMode === "localStorage" && typeof window !== "undefined" && !!window.localStorage;
        this.sessionKey = "".concat(this.baseKey, "::").concat(SESSION_TOKEN_SUFFIX);
        this.sessionStore = this.useLocalStorage ? makeLocalStore(this.sessionKey) : makeMemoryStore();
    }
    getStore(audience, scope) {
        const key = "".concat(this.baseKey, "::").concat(JSON.stringify([ audience !== null && audience !== void 0 ? audience : "", scope !== null && scope !== void 0 ? scope : "" ]));
        if (!this.slots.has(key)) {
            this.slots.set(key, this.useLocalStorage ? makeLocalStore(key) : makeMemoryStore());
        }
        ...
        if (this.useLocalStorage) {
            try {
                ...
                for (let i = 0; i < window.localStorage.length; i++) {
                    ...
                    if (key === null || key === void 0 ? void 0 : key.startsWith(prefix)) stale.push(key);
                }
...
class AnonymousSessionApiClient {
    constructor(authJsClient, clientId) {
        let cacheMode = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "localStorage";
        let lockManager = arguments.length > 3 ? arguments[3] : undefined;
        this.authJsClient = authJsClient;
        this.clientId = clientId;
        this.cache = new AnonymousSessionCacheManager(clientId, cacheMode);
        this.lockManager = lockManager !== null && lockManager !== void 0 ? lockManager : getLockManager();
    }
    async createSession(options) {
        const session = await this.authJsClient.createSession(options);
        this.cache.setSessionToken(Object.assign({
            sessionToken: session.sessionToken
        }, session.sessionTokenExpiresAt !== undefined && {
            sessionTokenExpiresAt: session.sessionTokenExpiresAt
        }));
        this.cache.getStore(options === null || options === void 0 ? void 0 : options.audience, options === null || options === void 0 ? void 0 : options.scope).set(Object.assign({
    ...
    async getTokenSilently(options) {
        const store = this.cache.getStore(options === null || options === void 0 ? void 0 : options.audience, options === null || options === void 0 ? void 0 : options.scope);
        const slot = store.get();
        if (slot && slot.expiresAt - EXPIRY_LEEWAY_SECONDS > Date.now() / 1e3) {
            return slot;
        }
        return this.lockManager.runWithLock("anonymous::".concat(this.clientId), 5e3, async () => {
            var _a;
            const afterLock = store.get();
            if (afterLock && afterLock.expiresAt - EXPIRY_LEEWAY_SECONDS > Date.now() / 1e3) {
                return afterLock;
            }
            const sessionToken = (_a = this.cache.getSessionToken()) === null || _a === void 0 ? void 0 : _a.sessionToken;
            const session = await this.authJsClient.getAccessToken(Object.assign(Object.assign({}, options), {
                sessionToken: sessionToken
            }));
            this.cache.getStore(options === null || options === void 0 ? void 0 : options.audience, options === null || options === void 0 ? void 0 : options.scope).set(Object.assign({
                accessToken: session.accessToken,
    ...
    hasSession() {
        var _a;
        return !!((_a = this.cache.getSessionToken()) === null || _a === void 0 ? void 0 : _a.sessionToken);
    }
    getClaims() {
        return null;
    }
...
class Auth0Client {
    ...
    constructor(options) {
        ...
        this.anonymous = new AnonymousSessionApiClient(this.authJsClient.anonymous, this.options.clientId, this.options.anonymousSessionsCacheMode, this.lockManager);
    ...
    async _maybeCreateAnonymousSession() {
    ...
    async checkSession(options) {
        ...
        try {
            ...
            if (e instanceof GenericError && e.error === "login_required" && e.error_description !== MFA_STEP_UP_ERROR_DESCRIPTION) {
                await this._maybeCreateAnonymousSession();
            }
        }
    }
...
exports.AnonymousSessionApiClient = AnonymousSessionApiClient;

exports.AnonymousSessionClient = AnonymousSessionClient;

exports.AnonymousSessionError = AnonymousSessionError;
